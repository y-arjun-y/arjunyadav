...
export async function getStaticPaths() {
  const files = fs.readdirSync("./blog-posts/drafts");
  const paths = files.map((fileName) => ({
    params: {
      draft: fileName.replace(".md", ""),
    },
  }));
  return {
    paths,
...
export async function getStaticProps({ params: { draft } }) {
  const fileName = fs.readFileSync(`./blog-posts/drafts/${draft}.md`, "utf-8");
